...
// A full-res Supabase art URL (.../tcg-art/<id>.png) → its lightweight thumbnail,
// served from the repo on Vercel's CDN at /tcg-art/thumb/<id>.jpg (~75 KB vs
// ~2.6 MB, off Supabase egress). Falls back to full-res if the pattern misses.
function thumbUrl(art) {
  return /\/tcg-art\/[^/]+\.png$/.test(art)
...
async function loadCards() {
  ...
  const pokes = POKEMON
    ...
    .map((c) => ({
      id: c.id, name: c.name, art: c.art, thumb: thumbUrl(c.art), type: c.type,
  ...
  const trainers = TRAINERS
    ...
    .map((c) => ({
      id: c.id, name: c.name, art: c.art, thumb: thumbUrl(c.art), type: c.kind,
...
function open(i){cur=i;const c=view[i];const img=document.getElementById('lbImg');
 img.src=c.thumb||c.art;img.alt=c.name; // thumb is already cached from the grid → instant
 if(c.art&&c.art!==c.thumb){const full=new Image();full.onload=()=>{if(cur>=0&&view[cur]&&view[cur].id===c.id)img.src=c.art;};full.src=c.art;}
